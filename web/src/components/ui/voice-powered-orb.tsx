...
interface VoicePoweredOrbProps {
  ...
  mainColor?: string; // Hex color string, e.g. "#FF0000"
  centerText?: string;
  isTitle?: boolean;
}
...
export const VoicePoweredOrb: FC<VoicePoweredOrbProps> = ({
  ...
  // Helper to convert hex to normalized RGB vec3
  ...
  const processedMainColor = mainColor ? hexToRGB(mainColor) : [0, 0, 0];
  const hasMainColor = !!mainColor ? 1.0 : 0.0;
  ...
  const frag = /* glsl */ `
    ...
    const vec3 defaultColor1 = vec3(0.611765, 0.262745, 0.996078);
    const vec3 defaultColor2 = vec3(0.298039, 0.760784, 0.913725);
    const vec3 defaultColor3 = vec3(0.062745, 0.078431, 0.600000);
    ...
    vec4 draw(vec2 uv) {
      ...
      if (uHasMainColor > 0.5) {
        color1 = uMainColor;
        color2 = mix(uMainColor, vec3(1.0), 0.3); // Lighter
        color3 = mix(uMainColor, vec3(0.0), 0.3); // Darker
      } else {
        color1 = adjustHue(defaultColor1, hue);
        color2 = adjustHue(defaultColor2, hue);
        color3 = adjustHue(defaultColor3, hue);
      }
    ...
    try {
      ...
      program = new Program(glContext as any, {
        ...
        uniforms: {
          ...
          uMainColor: { value: new Vec3(...processedMainColor) },
          uHasMainColor: { value: hasMainColor },
        },
      });
      ...
      const update = (t: number) => {
        ...
        // Update new uniforms dynamically
        program.uniforms.uMainColor.value.set(...processedMainColor);
        program.uniforms.uHasMainColor.value = hasMainColor;
      ...
      return () => {
        ...
        if (container && glContext && glContext.canvas) {
          try {
            if (container.contains(glContext.canvas as Node)) {
              container.removeChild(glContext.canvas as Node);
            }
          } catch (error) {
